      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {signals.map((signal, index) => (
          <div key={index} className="flex flex-col items-center text-center">
            ...
            <div className="text-xs text-gray-600 font-medium">{signal.description}</div>
          </div>
        ))}
      </div>
    </div>
...
export const UrgencyIndicators: React.FC<{ 
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {/* Spots Remaining */}
      {spotsRemaining <= 5 && (
        <div className="flex items-center gap-2 text-sm bg-red-50 border-2 border-red-300 rounded-lg p-3">
          <Clock className="w-4 h-4 text-red-600" />
          <span className="text-red-900 font-medium">
            <span className="font-bold">Only {spotsRemaining} spots remaining</span> for this departure
          </span>
        </div>
      )}
      ...
      {/* Other Viewers */}
      {viewersCount > 0 && (
        <div className="flex items-center gap-2 text-sm bg-blue-50 border-2 border-blue-300 rounded-lg p-3">
          <div className="w-2 h-2 bg-blue-600 rounded-full animate-pulse"></div>
          <span className="text-blue-900 font-medium">
            <span className="font-bold">{viewersCount} other travelers</span> are viewing this tour
          </span>
        </div>
      )}
      ...
      {priceIncrease && (
        <div className="flex items-center gap-2 text-sm bg-yellow-50 border-2 border-yellow-300 rounded-lg p-3">
          <RefreshCw className="w-4 h-4 text-yellow-600" />
          <span className="text-yellow-900 font-medium">
            Prices may increase soon. <span className="font-bold">Book now to secure current rate</span>
          </span>
        </div>
      )}
    </div>
