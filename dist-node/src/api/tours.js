...
const mockTours = [
    ...
    // Route A Preferred
    {
        ...
        summary: "14-day journey through France, Switzerland, Italy, and Vatican City, (international flights to/from Manila handled separately).",
        line: "ROUTE_A",
        durationDays: 14,
...
export async function fetchContinents() {
    ...
    const countries = mockTours.reduce((acc, t) => acc.concat(t.additionalInfo?.countriesVisited ?? []), []);
...
export async function fetchToursByCountry(country) {
    await new Promise((r) => setTimeout(r, 180));
    return mockTours.filter((t) => (t.additionalInfo?.countriesVisited ?? []).indexOf(country) !== -1);
}
export async function fetchToursByContinent(continent) {
    await new Promise((r) => setTimeout(r, 180));
    const continentCountries = new Set(Object.keys(countryToContinent).filter((country) => countryToContinent[country] === continent));
    return mockTours.filter((t) => (t.additionalInfo?.countriesVisited ?? []).some((c) => continentCountries.has(c)));
}
